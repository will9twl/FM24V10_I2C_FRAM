...
uint32_t FRAM_MultiByte_Write(I2C_HandleTypeDef *hi2c, uint32_t mem_addr, uint8_t *pData, uint16_t size)
{
  ...
  if(mem_addr >= 0x10000){ //memory address points to 10000h to 1FFFFh
    device_addr_write = FRAM_SLAVE_DEV0_ADDR | FRAM_PAGE_SELECT_H | FRAM_WRITE_TO_SLAVE;  
  }else{ //memory address points to 00000h to 0FFFFh
    device_addr_write = FRAM_SLAVE_DEV0_ADDR | FRAM_PAGE_SELECT_L | FRAM_WRITE_TO_SLAVE;  
  }
...
uint32_t FRAM_MultiByte_Selective_Read(I2C_HandleTypeDef *hi2c, uint32_t mem_addr, uint8_t *pData, uint16_t size)
{
  ...
  if(mem_addr >= 0x10000){ //memory address points to 10000h to 1FFFFh
    device_addr_write = FRAM_SLAVE_DEV0_ADDR | FRAM_PAGE_SELECT_H | FRAM_WRITE_TO_SLAVE;  
    device_addr_read = FRAM_SLAVE_DEV0_ADDR | FRAM_PAGE_SELECT_H | FRAM_READ_FROM_SLAVE;
  }else{ //memory address points to 00000h to 0FFFFh
    device_addr_write = FRAM_SLAVE_DEV0_ADDR | FRAM_PAGE_SELECT_L | FRAM_WRITE_TO_SLAVE;  
    device_addr_read = FRAM_SLAVE_DEV0_ADDR | FRAM_PAGE_SELECT_L | FRAM_READ_FROM_SLAVE;
  }
...
uint32_t FRAM_Read_Unique_Serial_Num(I2C_HandleTypeDef *hi2c, uint8_t *pData, uint16_t size) //read 8-byte serial number
{
  ...
  if(hi2c->State == HAL_I2C_STATE_READY){
    ...
    /* Send Slave Address */
    if(I2C_MasterRequestWrite(hi2c, (uint8_t)0xF8, 1) != HAL_OK) //send rsvd slave ID F8h
    {
      if(hi2c->ErrorCode == HAL_I2C_ERROR_AF)
      {
        /* Process Unlocked */
    ...
    uint32_t Size = 1, i = 0;
    uint8_t memory_addr[] = {
      FRAM_SLAVE_DEV0_ADDR, //slave address      
    };
    while(Size > 0)
    {
      /* Wait until TXE flag is set */
    ...
    /* Send Slave Address */
    if(I2C1_MasterRequestRead(hi2c, (uint8_t)0xCD, 1) != HAL_OK) //send rsvd slave ID CDh
    {
      if(hi2c->ErrorCode == HAL_I2C_ERROR_AF)
      {
        /* Process Unlocked */
...
uint32_t FRAM_Enter_Sleep_Mode(I2C_HandleTypeDef *hi2c) //FRAM enter sleep mode (auto-wakeup when identified with Slave Addr)
{
  ...
  if(hi2c->State == HAL_I2C_STATE_READY){
    ...
    /* Send Slave Address */
    if(I2C1_MasterRequestRead(hi2c, (uint8_t)0x86, 1) != HAL_OK) //send rsvd slave ID 86h
    {
      if(hi2c->ErrorCode == HAL_I2C_ERROR_AF)
      {
        /* Process Unlocked */
